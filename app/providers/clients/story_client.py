...
9. **아이가_읽은_동시 가 있으면 그 시를 「 」 안에 그대로 옮겨 적는다.**
   "동시를 읽었어요" 로만 적고 넘어가지 않는다 — 동화책을 나중에 다시 펼쳤을 때
   그날 무슨 시를 읽었는지 알 수 있어야 하고, 그것이 이 장면의 내용 전부다.
   아이가_센_것 이 있으면 마찬가지로 무엇을 몇 개 세었는지 그대로 적는다.
   옮겨 적은 「 」 안의 글은 3번의 문장 수·길이 제한에서 뺀다.
