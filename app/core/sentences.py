...
#: 수학(math)에는 있다 — 고른 문장을 통째로 읽지만, 발음이 약한 어절 하나를 비우는
#: 빈칸 퀴즈가 그 뒤에 붙는다.
SENTENCE_TRANSLATION_PARTS: dict[SentenceId, dict[str, list[tuple[str, list[int]]]]] = {
    ...
    # ── 수학: 과일 개수 세기 ────────────────────────────────────────────
    #
    # 어절이 1:1 로 떨어진다. 베트남어와 중국어 둘 다 분류사를 따로 쓰기 때문이다 —
    # "개" 가 quả / 个·根 에 그대로 대응한다.
    #
    #   사과가(0)  한(1)  개(2)  있어요(3)
    #   táo(0)     một(1) quả(2) Có(3)
    #   苹果(0)     一(1)  个(2)  有(3)
    #
    # 어순이 한국어와 정반대라(있다-수-분류사-사물) 조각 순서도 그대로 뒤집힌다.
    # 이 표가 없으면 아이가 전구를 눌러도 **빈칸이 모국어의 어느 말인지 안 짚힌다** —
    # 뜻만 통째로 뜨고 끝난다.
    SentenceId.MATH_1: {
        "vi": [
            ("Có", [3]),
